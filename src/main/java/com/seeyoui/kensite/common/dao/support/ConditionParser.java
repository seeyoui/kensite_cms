...
public class ConditionParser {
	...
	public String parse(boolean includeOrder) {
		...
		if (condition.getConditions().size() > 0) {
			HqlAssemblers hqlAssembler = new HqlAssemblers(" where ", " ");
		...
		if (includeOrder && condition.getOrders().size() > 0) {
			HqlAssemblers hqlAssembler = new HqlAssemblers(" order by ", ", ");
	...
	private void joinHqlAndParams(Expression expression, HqlAssemblers hqlAssembler, List<Object> params) {
		/*
