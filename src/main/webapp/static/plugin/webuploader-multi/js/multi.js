jQuery(function() {
	...
	uploader = WebUploader.create({
		...
		server : ctx + '/sys/uploadfile/uploadReal',
		fileNumLimit : 10,
		fileSizeLimit : 5 * 1024 * 1024, // 200 M
		fileSingleSizeLimit : 1 * 1024 * 1024, // 50 M
		formData : {
			url : $('#uploader .queueList').data('url')
		}
	});
	...
	function addFile(file) {
		if (parent.checkUploadFile(file.name)) {
			file.setStatus('invalid');
			file.statusText = 'exist';
		}
		...
		if (state === 'ready') {
			...
			if (stats.uploadFailNum) {
				text = '已成功上传'
						+ stats.successNum
						+ '张照片至'
						+ $('#uploader .queueList').data('url')
						+ '，'
						+ stats.uploadFailNum
						+ '张照片上传失败，<a class="retry" href="#">重新上传</a>失败图片<!--或<a class="ignore" href="#">忽略</a>-->'
			}
	...
	uploader.on('uploadSuccess', function(file, response) {
		layer.msg("上传成功！", {
			offset : layerMsgOffset,
			icon : 6,
			shift : 8,
			time : layerMsgTime
		});
		parent.afterUploadFile(file);
	});
